...
refined<- cbind(tempdata,surf_enviro[,c(9:20)])

lapply(rivers, standi, data = refined)

standi<-function(x){
    scale(data=subset.data.frame(refined, river==rivers[[x]], select = c(-river, -zone)), 
          center = TRUE, scale = TRUE)
}
standi("Madison")

                 a=as.numeric(nrow(x),b=as.numeric(ncol(x)),data=refined)){
  river<- data.frame(matrix(0,ncol=b,nrow=a)) 
  river2<- data.frame(matrix(0,ncol=b,nrow=a)) # or other
...
dset = subset.data.frame(x, river=="Boulder", select = -river)
river = dset[,c(8:19)]
